...
def nn_build(seed,layerlist, nonlin='sigmoid', eta=0.01, init='glorot'):
    ...
    addadadelta(nn)
    zs = nn['weights'][1].shape
    nn['front_weights'].append(np.random.rand(zs[1],zs[0]))
    return nn
...
def gradient(nn, delta):
    ...
    nabla_w.append(dW)
    for i in range(len(nn['weights']) - 2, 0, -1):
        dact = nn['nonlin'][i][1]
        ...
        nabla_w.append(dW)
    dact = nn['nonlin'][0][1]
    delta = np.dot(nn['front_weights'][0], delta * dact(nn['zs'][1]))
    dW = np.outer(delta, nn['activations'][0][0].T)#average_gradient(delta,nn['activations'][0])
    nabla_b.append(np.mean(delta*dact(nn['zs'][0]),axis=1))
...
def label_bin(labels):
    matr = np.zeros([labels.shape[0],10])
    print labels
...
def master_node(nn,data,labels):
    ...
    for n in range(len(nn)):
        #print labels[n]
        r = forward(nn[n], np.reshape(data[n],(1,784)))
        print labels[n]
