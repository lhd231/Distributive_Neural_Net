...
def visitClassicBatches(nn,data,labels, val_d, val_l, list, it=1000):
    for c in range(it):
      ...
      if c %10 == 0:
        acc = accuracy(nn, val_d, val_l, thr=.5)
        list[c/10] = acc
        print acc
...
def new_acc(nn, data, label, thr = 0.5):
  predict = []
  for i in range(len(data)):
    if np.argmax(nnS.forward(nn,data[i].T)) == label[i].T:
      predict.append(True)
    else:
...
classAcc1 = [0 for i in range(100)]
horn1Acc = [0 for i in range(100)]
horn2Acc = [0 for i in range(100)]
middleAcc = [0 for i in range(100)]
number_of_sites = 3
sample_size = 10
def sing_run(te):
    print te
    ...
    minim = min(len(total_data[0]),len(total_data[1]))
    ...
    nnDecent = nnS.nn_build(1,[784,10,10,3],eta=eta,nonlin="tanh")
    ...
    visitClassicBatches(nnTogetherClassic,centralized_data,centralized_label,validation_data, validation_label,classAcc1,it=iters)
    print "finished cents"
    visitClassicBatches(nnHorn1,horn1_data,horn1_label,validation_data, validation_label,it=iters)
    print "finished horn1"    
    visitClassicBatches(nnHorn2,horn2_data,horn2_label,validation_data, validation_label,it=iters)
    visitClassicBatches(nnMiddle,middle_data,middle_label,validation_data, validation_label,it=iters)
...
np.savetxt("decent-exp1.txt",nn1Acc)
np.savetxt("cent-exp1.txt",classAcc1)
np.savetxt("ones-exp1.txt",horn1Acc)
np.savetxt("zeros-exp1.txt",horn2Acc)
np.savetxt("sevens-exp1.txt",middleAcc)
