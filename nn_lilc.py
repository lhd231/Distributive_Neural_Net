...
def nn_build(seed,layerlist, nonlin='sigmoid', eta=0.01, init='glorot'):
    ...
    ws = nn['weights'][1].shape
    nn['R'] = np.random.randn(ws[1],ws[0])
    addadadelta(nn)
    return nn
...
def backprop(nn, delta):
    ...
    dact = nn['nonlin'][0][1]
    delta = np.dot(nn['weights'][1], delta * dact(nn['zs'][1]))

    delta = np.reshape(delta[:,0],(20,1))
    dW = delta 
    
    #print average_gradient(delta,nn['activations'][0]).shape
    front_b.append(np.mean(delta*(nn['zs'][0]),axis=1))
    front_w.append(dW)   
    for i in range(1,len(nn['weights'])):
        nn['weights'][i] -= eta * nabla_w[-i]
        nn['biases'][i] -= eta * nabla_b[-i]
    nn['weights'][0] -= eta * front_w[0]
    nn['biases'][0] -= eta * front_b[0]
