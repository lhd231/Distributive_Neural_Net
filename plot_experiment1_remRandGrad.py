...
dif3 = np.subtract(1,np.divide((np.loadtxt("decent-remGrad-s1-moons.txt")[:,:40][:,::2]),100))
dif2 = np.subtract(1,np.divide((np.loadtxt("decent-remGrad-s2-moons.txt")[:,:40][:,::2]),100))

dif1 = np.subtract(1,np.divide((np.loadtxt("decent-remGrad-s3-moons.txt")[:,:40][:,::2]),100))
cent = np.subtract(1,np.divide(np.loadtxt("cent-remGrad-moons.txt")[:,:40][:,::2],100))
c2 = np.subtract(1,np.divide(np.loadtxt("eights-remGrad-moons.txt")[:,:40][:,::2],100))
c1 = np.subtract(1,np.divide(np.loadtxt("sevens-remGrad-moons.txt")[:,:40][:,::2],100))
c3 = np.subtract(1,np.divide(np.loadtxt("zeros-remGrad-moons.txt")[:,:40][:,::2],100))
...
cols = range(10,410,20)
s = pd.DataFrame(fw, columns=cols)
...
df_long = pd.melt(s,"class", var_name="Iterations", value_name="Accuracy")
#print df_long
...
g = seaborn.factorplot(x="Iterations", y="Accuracy", hue="class",data=df_long,kind="box",palette=pal, size =6, aspect=2, legend = False)
g.despine(left=True)
plt.legend(loc='lower right')
#g.set_ylabels("survival probability")
plt.ylim([0,1])
#seaborn.factorplot(x="sample size", y="error rate",data=df_long, kind="box")
seaborn.plt.savefig("moons_remRandGrad_iters.svg",bbox_inches="tight",pad_inches=0)
seaborn.plt.show(g)
