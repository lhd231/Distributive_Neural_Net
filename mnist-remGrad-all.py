...
def sing_run(te):
    ...
    nnTogetherClassic = nnS.nn_build(1,[784,20,20,10],eta=eta,nonlin="tanh")
    ...
    nnDecent = []
    for i in range(6):
      nnDecent.append(nnDif.nn_build(1,[784,20,20,10],eta=eta,nonlin="tanh"))
    ...
    horn1_data,horn1_label = [x for x in iter_minibatches(1,total_data[0][:50],total_label[0][:50])]
    horn2_data,horn2_label = [x for x in iter_minibatches(1,total_data[0][50:100],total_label[0][50:100])]
    middle_data,middle_label = [x for x in iter_minibatches(1,total_data[0][100:150],total_label[0][100:150])]
    ...
    batches_decent_data, batches_decent_label = [x for x in iter_minibatches(6,total_data[0],total_label[0])]
...
np.savetxt("decent-remGrad-all-s1-5.txt",nn1Acc[0])
np.savetxt("decent-remGrad-all-s2-5.txt",nn1Acc[1])
np.savetxt("decent-remGrad-all-s3-5.txt",nn1Acc[2])
np.savetxt("cent-remGrad-all-5.txt",classAcc)
#visitClassicBatches(nnHorn1,horn1_data,horn1_label,validation_data, validation_label,eights,te,it=iters)
print "finished horn1"
np.savetxt("eights-remGrad-all-5.txt",eights)    
#visitClassicBatches(nnHorn2,horn2_data,horn2_label,validation_data, validation_label,sevens,te,it=iters)
np.savetxt("sevens-remGrad-all-5.txt",sevens)    
#visitClassicBatches(nnMiddle,middle_data,middle_label,validation_data, validation_label,zeros,te,it=iters)
np.savetxt("zeros-remGrad-all-5.txt",zeros)
