...
def split(data, label):
  ...
  print len(return_list_data[2])
  print len(return_list_label[2])
  
  #plt.scatter(return_list_data[0]+return_list_data[1]+return_list_data[2],return_list_label[0]+return_list_label[1]+return_list_label[2],s=40)
  #plt.show()
...
def visitbatches(nn, data, labels, it=1000):
    for c in range(it):
...
nn1Acc = [[[0 for i in range(50)] for j in range(10)] for k in range(3)]
...
np.savetxt("new-3-site-decent-s1-x.txt",nn1Acc[0])
np.savetxt("new-3-site-decent-s2-x.txt",nn1Acc[1])
np.savetxt("new-3-site-decent-s3-x.txt",nn1Acc[2])
np.savetxt("new-3-site-cent-x.txt",classAcc1)
np.savetxt("new-3-site-cent-horn1-x.txt",horn1Acc)
np.savetxt("new-3-site-cent-horn2-x.txt",horn2Acc)
np.savetxt("new-3-site-cent-middle-x.txt",middleAcc)
