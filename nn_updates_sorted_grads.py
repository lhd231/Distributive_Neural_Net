...
def average_first_gradient(deltas, activations):
    dW = []
    grad_pick = random.randint(0,2)
    for i in range(deltas.shape[1]):

        dW.append(np.outer(deltas[:,i], activations[:,i].T))

    #Build the absolute value so we take largest change
    abs_val = np.absolute(dW[grad_pick])
    
    #Build a dict of indices to cell values
    d_c = dict(((i,j), abs_val[i][j]) for i in range(abs_val.shape[0]) for j in range(abs_val.shape[1]))
    
    #Sort the dict and pick only the first 200 items
    sorted_x = sorted(d_c.items(), key=operator.itemgetter(1), reverse=True)[:1000]

    #Build a matrix of zeros
    matr = np.zeros([abs_val.shape[0],abs_val.shape[1]])

    #Find the appropriate cells based on the sorted list and set them to one
    for item in sorted_x:
        matr[item[0][0]][item[0][1]] = 1
    output = 0
    
    #Average our gradients while mulitplying them by our binary matrix,
    #Giving us gradients that are either averaged, or zero
    for item in dW:
        output += item * matr
    return output / 3
...
def backprop(nn, delta):
    ...
    for i in range(len(nn['weights']) - 2, -1, -1):
        ...
        if i == 0:
            dW = average_first_gradient(delta,nn['activations'][i])
        else:
