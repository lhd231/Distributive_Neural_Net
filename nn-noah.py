...
class neuralNetwork:
    ...
    def batch(self,seed,Z,y,groupSize):
        l = random.sample(xrange(len(Z)),groupSize)
        ...
        for num in l:
            batchX.append(Z[num])
            batchY.append(y[num])
        return np.asarray(batchX),np.asarray(batchY).reshape(groupSize,1)
    ...
    def back_propagation(self, dW1, db1, dW2, db2):
        # Backpropagation
...
for i in range(len(img_data)):
    if labels[i] == 8:
        ...
        img = img_data[i]
        imgs.append(img)
#X and y hold our data and label data
#We split them into Z1, Z2, y1, and y2 for two NNs

X = np.asarray(imgs, np.dtype(float))

print "after load"
X, y = datasets.make_moons(200, noise=0.40)
...
print "after net 2"
model = {}
#Here we call the forward prop, get the deltas, average them
#then send that through the back prop of the individual NNs
for i in range(len(Z1) *30):
    dW1_1, db1_1, dW2_1, db2_1 = NN.forward_propagation(i)
    dW1_2, db1_2, dW2_2, db2_2 = NN2.forward_propagation(i)
    
    avgdw = (dW1_1 + dW1_2)/2
    avgdb = (db1_1 + db1_2)/2
    avgdw2 = (dW2_1 + dW2_2)/2
    avgdb2 = (db2_1 + db2_2)/2
    NN.back_propagation(avgdw,avgdb,avgdw2,avgdb2)
    NN2.back_propagation(avgdw,avgdb,avgdw2,avgdb2)
print "after process"
model = NN2.get_model()
